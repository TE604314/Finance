...
--master maitain by version,get newest version
delete ss_rs_ts_aut_ap_digital_schema.Dim_Finance_FPA_ARBAD_OriginalActiveMaster;
insert into ss_rs_ts_aut_ap_digital_schema.Dim_Finance_FPA_ARBAD_OriginalActiveMaster
(
"Customer",
"Company Code",--
"Name",	--
...
BU,
Num,
row_number()over(partition by "Company Code","Customer" order by Num desc) as ranks
from
(
select 
"Customer",
...
"Payment term description",
BU,
replace(replace(left(filepath,7),'F',''),'Q','0') as Num
...
where fiscal_year_month_id>=202311 and fiscal_year_week_id<=
(
...
where concat(reporting_fiscal_year,right(reporting_fiscal_week+100,2)) >=202352 and company_code in 
('1473','0451','1321','1641','1908','3004','3027','2554') and business_unit_id in ('AUT','HES')
group by 
...
select 
FiscalYM,
max(COALESCE(cast(rate as float),0)) as  PastdueARPercentage_rate
...
"Sold To Customer Number",	
"Sold To Worldwide Customer Account Number",
case when c."WW Group" is not null then c."WW Group" else "Sold To Worldwide Customer Account Name" end as "Sold To Worldwide Customer Account Name",	
"Sold To Customer Name",
b.BU,
d.Visual_Payment_term,
timestamps
from 
(
select 
...
from ss_rs_ts_aut_ap_digital_schema.Dim_Finance_FPA_ARBAD_OriginalActiveMaster
GROUP BY Customer
) b
...
where "PGL group"<>'PGL Group' and "WW Group" is not null and "WW Group"<>''
group by "Sold-to"
) c
on a."Sold To Customer Number"=c.Customer

left join 
(
select
"WW Group",
max("Payment_term") as Visual_Payment_term
from s3dataschema.ods_Finance_FPA_ARBAD_WWCustomerGroup
group by "WW Group"
) d
on c."WW Group"=d."WW Group";
--Customermasterdata
...
"Sold To Customer Number",
"Company Code" ,	
max("Sold To Customer Number") as Name,
null as "Terms of Payment",
null as "Payment term description",--
null as BU--
from ss_rs_ts_aut_ap_digital_schema.DWD_Finance_FPA_ARBAD_WeeklyAR a
where not EXISTS (select distinct "Customer","Company Code" from ss_rs_ts_aut_ap_digital_schema.Dim_Finance_FPA_ARBAD_OriginalActiveMaster b
where Customer<>'Customer' and a."Sold To Customer Number"=b. "Customer" and a."Company Code"=b."Company Code")
group by "Sold To Customer Number",
"Company Code"
...
select distinct "Sold To Worldwide Customer Account Name" 
from ss_rs_ts_aut_ap_digital_schema.Dim_Finance_FPA_ARBAD_WorldwideCustomer
where "Sold To Worldwide Customer Account Name" like '%NIO%'
(
"Sold To Customer Number",	
"Sold To Worldwide Customer Account Number",
"Sold To Worldwide Customer Account Name",	
