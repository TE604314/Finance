call ss_rs_ts_aut_ap_digital_schema.sp_Finance_Plant_Losspn_ODS_to_DWD_fbl3n()
...
select 
    ...
    case when account<>'5330050' then 1 else 0 end  as Movetypelink,
 	case when account in ('6060071','5009022') then '备件报废'
     when account in ('5340050') then '存货报废'
     when account in ('5330055') then '盘盈盘亏'
	 when account in ('5330050') then '尾数调整'
     when account in ('5013004') then '索赔'
     end as "Loss_Type",
    "document number",
    "document date",
    ...
    "assignment",
	text,
    case when quantity='' or quantity is null then 0 else cast(quantity as float) end as quantity,
...
max("procurement type") as "Procurement type",
max("Material type") as "Material type"
from s3dataschema.ods_finance_masterdata_067 
group by 
plant_id,
material_number
) b
...
on a.plant=b1.plant_id and a.material=b1.material_number

left join 
(
select  
material_number,
max(material_type_code) as "Material type"
from master_data.dimension_material_current
group by material_number
) b2
on a.material=b2.material_number


--master_data.dimension_material_plants_current 
left join 
...
from FBL3N_Step1
where "Procurement type" in ('F','X') and reason_for_movement_code not in ('0019','1021') AND account<>'5330050' and  account<>'5013004'
 and quantity<>'' and quantity is not null and 
case when account in ('5330050') and quantity<0 then 0 else 1 end=1
) a 
...
quantity_QTD,
Amount_QTD,
b.purchasing_document_id,
b.invoice_posting_date,
b.invoice_quantity,
b.invoice_document_currency_amount
from
(
select
   Yearquarter,
  ...
  "posting date",
   ...
   "amount in local currency",
inventory_movement_type_code,
sum(quantity) over(partition by Yearquarter,account,plant,material order by "posting date" asc rows between unbounded preceding and current row) as quantity_QTD,
sum("amount in local currency") over(partition by Yearquarter,account,plant,material order by "posting date" asc rows between unbounded preceding and current row) as Amount_QTD
from
(
select
  left("posting date",4)+concat('Q',left(DATE_PART(QUARTER, "posting date"),1)) as Yearquarter,
   plant,
   material,
   account,
  "posting date",
...
where "Procurement type" in ('F','X') and reason_for_movement_code not in ('0019','1021') AND account<>'5330050' and account<>'5013004'
 and quantity<>'' and quantity is not null and 
case when account in ('5330050') and quantity<0 then 0 else 1 end=1 AND material='1927323-2'
group by 
plant,
   material,
   account,
...
SUM(invoice_document_currency_amount) AS invoice_document_currency_amount
from ss_rs_ts_aut_ap_digital_schema.DWD_finance_plant_losspn_PO
where material='1927323-2' and TO_CHAR(TO_DATE(invoice_posting_date, 'YYYY-MM-DD'), 'YYYYMMDD')>'20251001'
GROUP BY plant,
material,
purchasing_document_id,
invoice_posting_date
) b
on a1.plant=b.plant and a1.material=b.material and a1."posting date">=b.invoice_posting_date;
...
select
  ...
  inventory_movement_type_code,
   ...
   max("amount in local currency") as "amount in local currency",
   max(quantity_QTD) as quantity_QTD,
   max(Amount_QTD) as Amount_QTD,
    listagg(purchasing_document_id,'/') as purchasing_document_id,
	listagg(invoice_posting_date,'/')  as invoice_posting_date,
	sum(invoice_quantity) as invoice_quantity,
	sum(invoice_document_currency_amount) as invoice_document_currency_amount
from Temp_1
group by 
...
plant,
material,
listagg(purchasing_document_id,'/')
from ss_rs_ts_aut_ap_digital_schema.DWD_finance_plant_losspn_PO
where material='1927323-2'
group by 
plant,
material;
